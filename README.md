...
4. Enter a friendly name for the application, for example 'TodoListWebApp_MT' and select 'Web Application and/or Web API' as the Application Type. For the sign-on URL, enter the base URL for the sample, which is by default `https://localhost:44376/`. Click on **Create** to create the application.
...
7. In the same page, change the `Logout Url` property to `https://localhost:44376/Account/EndSession`.  This is the default single sign out URL for this sample.
8. Find "Multi-tenanted" switch and flip it to "yes", then click the Save option at the bottom of the page.
